...
export class RestaurantsService {
  ...
  async findFilterAll(
    ...
    try {
      ...
      const nextPage = page < totalPages ? `https://${host}/api/v1/restaurants?page=${page + 1}&perPages=${perPages}` : null;
      const prevPage = page > 1 ? `https://${host}/api/v1/restaurants?page=${page - 1}&perPages=${perPages}` : null;
